...
if elements.key?(element)
	puts 'Порядковый номер: ' + elements[element]['number'].to_s
	puts 'Название: ' + elements[element]['name']
	puts "Первооткрыватель: #{elements[element]['discoverer']} (#{elements[element]['year']} г.)"
	puts "Плотность: #{elements[element]['density'].to_s} г/см³"
